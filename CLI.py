...
def handle_online_input(input_text, selected_option, output_selected_option, sel_lang_option):
    ...
    if time > 300:
        ...
        for i in range(0, time, 300):
            ...
            new_name = convert_to_wav(filename)
            model_process(new_name, selected_option, output_selected_option, sel_lang_option, count)
            count += 1
    else:
        print("下載中...")
...
def process_local_file(input_text, selected_option, output_selected_option, sel_lang_option):
    ...
    if method:
        new_name = method()
        model_process(new_name, selected_option, output_selected_option, sel_lang_option, 0)
    else:
        raise ValueError("無法處理的本地檔案格式")

def model_process(filename, selected_option, output_selected_option, sel_lang_option, count):
    process_audio = initialize_audio_model(selected_option, count)
    print("處理中...")
    run_model(process_audio, filename, sel_lang_option)
    try:
        print("輸出檔案中...")
        output_result(process_audio, filename, count, output_selected_option)
        print("完成處理")
    except Exception as e:
        print(f"處理結果時發生錯誤: {str(e)}")
        with open("error_log.txt", "a") as log_file:
            log_file.write(f"Output Error: {str(e)}\n")
